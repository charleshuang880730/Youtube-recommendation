...
var vm = new Vue({
    ...
    data:{
        ...
        buffer_items: [],
        tmp_items: [], // 處理後的結果影片
        keyword:["美食","運動","台北市","休閒","政府"], // 關鍵字推薦中的所有關鍵字形成的list
        all_vid:vids, //所有影片vid

        // 排序
        sort_mode: "相對娛樂程度",
        sort_choice_list: ["相對娛樂程度", "相對觀眾黏著度", "相對總角色人氣值", "相對含梗量", "相對基本面", "觀看次數", "按讚數量"], 
        attr_value_list: [],
    ...
    methods: {
        ...
        bindWordCloud(_id){
          ...
          .each(function(d,i){
              ...
              if(global_this.items[vid]["Word_Cloud"].length > 0){
                if(!d3.select(this).select(".chart img").empty()){
                  d3.select(this).select(".chart img").remove();
                }
              }
              ...
              if(global_this.items[vid]["Word_Cloud"].length == 0){
                if(!d3.select(this).select(".chart svg").empty()){
                  d3.select(this).select(".chart svg").remove();
                }
              }
          })
        },
        ...
        recommendKeyWordQuery(keyword){ // return vid_list
          ...
          console.log('||||||||||||||||||||||||| Send Query |||||||||||||||||||||||||!');
          console.log('||||||||||||||||||||||||| Waiting for Response |||||||||||||||||||||||||!');
          query_url = this.keyword_api+keyword;
          global_this = this;
        ...
        rangeValueQuery(value){  //return vid_list
          ...
          console.log('Send Query!');
          console.log('||||||||||||||||||||||||| Popularity API Start |||||||||||||||||||||||||!');
          ...
          .then(video_info_obj => {
            ...
            console.log('||||||||||||||||||||||||| Popularity API Finish |||||||||||||||||||||||||!');
        ...
        renderRadarChart(vid){
                ...
                type: 'radar',
                data: {
                labels: ["娛樂程度", "觀眾黏著度", "總主角人氣值","含梗量","基本面" ],
                datasets: [{
                label: "分項分數",
